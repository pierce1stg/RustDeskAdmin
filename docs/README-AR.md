...
- **لوحة إدارة الويب** (React SPA + Go API):
  ...
  - الأجهزة — قائمة الأجهزة المسجلة (`GET /api/devices`)، أعمدة PeerInfo
    (المضيف/المستخدم/النظام/الإصدار/الشاشات)، بحث في كل الحقول مع اختيار
    الأعمدة (يحتوي/تطابق تام/يبدأ بـ، والمعرفات العشرية مدعومة)، تثبيت/اسم
    مستعار، كلمات مرور محفوظة مشفّرة، عرض التفاصيل، الحذف. نقطة الحالة
    بثلاثة ألوان: متصل، غير متصل، غير مؤكد.
...
- **الحضور الحي** (حاوية `presence` + حكم الخلفية):
  - يقرأ اتصالات hbbs/hbbr الحقيقية مباشرةً من نطاقات الشبكة الخاصة بها
    (nsenter + `ss` + docker.sock) مع تقييم حداثة حركة كل مقبس؛
  - يكتب `status/presence.json` بعناوين IP للأقران النشطين وحالة منافذ الاستماع
    وربط الأقران بالعناوين؛
  - تسأل الخلفية أيضًا hbbs نفسه (طلب `OnlineRequest` ذاته الذي تستخدمه
    العملاء الأصلية) عن خريطة الاتصال الموثوقة لكل نظير — فلا يعود NAT
    المشترك يخلط بين الأقران (`HBB_ONLINE_ADDR`، الافتراضي `hbbs:21115`)؛
...
| GET     | `/api/devices`              | قائمة الأجهزة (مقسمة صفحات؛ `search` و`fields` و`ops`) |
